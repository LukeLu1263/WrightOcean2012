...
int Motion::main()
{
  // detect the "cognition bug"
  if(theCognitionToMotionReceiver.receivedNew())
  {
    Motion::lastCognitionPackageTimeStamp = SystemCall::getCurrentSystemTime();
  }
  // there has been no new package from Cognition in more than 500ms and thus we let the robot stand.
  else if(SystemCall::getCurrentSystemTime() - Motion::lastCognitionPackageTimeStamp > 500)
  {

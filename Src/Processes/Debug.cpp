...
int Debug::main()
{
  ...
  default:
    ...
    if(!theCognitionReceiver.isEmpty())
    {
      ...
      theLogCtrl.processBegin('c');
      theCognitionReceiver.handleAllMessages(theLogCtrl);
      theCognitionReceiver.moveAllMessages(theDebugSender);
    }
    if(!theMotionReceiver.isEmpty())
    {
      OUTPUT(idProcessBegin, bin, 'm');
      theLogCtrl.processBegin('m');
      theMotionReceiver.handleAllMessages(theLogCtrl);

...
int Cognition::main()
{
  ...
  if(CognitionLogDataProvider::isFrameDataComplete() && CameraProvider::isFrameDataComplete())
  {
    ...
    BH_TRACE_MSG("before TEAM_OUTPUT(idSensorData)");
    if(&Blackboard::theInstance->theFilteredSensorData && Global::getReleaseOptions().sensorData)
    {
      ...
      JointData j = Blackboard::theInstance->theJointRequest;
      TEAM_OUTPUT(idJointData, bin, j);
    }
...
bool Cognition::handleMessage(InMessage& message)
{
  ...
  default:
    return CognitionLogDataProvider::handleMessage(message) ||
           ...
           BH2011BehaviorControl::handleMessage(message) ||

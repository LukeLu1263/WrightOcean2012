...
void Regionizer::update(RegionPercept& rPercept)
{
	...
	vecCountFeasibleLeft.clear();
	vecCountFeasibleRight.clear();
	vecCountLeft.clear();
	vecCountRight.clear();
	for (int i = 0; i < 41; i++) 
	{
		vecCountFeasibleLeft.push_back(0);
		vecCountFeasibleRight.push_back(0);
		vecCountLeft.push_back(0);
		vecCountRight.push_back(0);
	}
  ...
  // Vision Based Obstacle Avoidance
  regionPercept->visionObstacleValid = false;
   if (fabs(toDegrees(theFilteredJointData.angles[JointData::HeadYaw])) < 5 && fabs(toDegrees(theFilteredJointData.angles[JointData::HeadPitch])-(-18)) < 5.0) 
   {
	regionPercept->ratioLeft = (double)countGreenLeft / countLeft;
	regionPercept->ratioRight = (double)countGreenRight / countRight;  

	regionPercept->vecCountFeasibleLeft = this->vecCountFeasibleLeft;
	regionPercept->vecCountFeasibleRight = this->vecCountFeasibleRight;
	regionPercept->vecCountLeft = this->vecCountLeft;
	regionPercept->vecCountRight = this->vecCountRight;

	regionPercept->visionObstacleValid = true;
  }  
}
...
void Regionizer::scanVertically()
{
  ...
  // Vision Based Obstacle Avoidance
  countGreenLeft = 0, countGreenRight = 0, countLeft = 1, countRight = 1;
  ...
  for(; x < xEnd && !perceptFull ; x += parameters.gridStepSize)
  {
	...
	// New Added by Alan, Vision-Based Obstacle avoidance	
	y = yStart;
	while(y < yEnd) {
		if (y < 65 || !(y > (-5*x+390) && y > (5*x-1210))) {
			//DOT("module:Regionizer:visionObstacle", x, y, ColorClasses::yellow, ColorClasses::yellow);
		} else {
			if (x < xEnd / 2) {
				if (pointExplorer.getColor(x,y) != ColorClasses::none) {
					countLeft++;
	
					this->vecCountLeft[(y-72)/parameters.gridStepSize]++;
				}
				if (pointExplorer.getColor(x,y) == ColorClasses::green || pointExplorer.getColor(x,y) == ColorClasses::orange) {
					countGreenLeft++;
					
					this->vecCountFeasibleLeft[(y-72)/parameters.gridStepSize]++;
				}
			} else {
				if (pointExplorer.getColor(x,y) != ColorClasses::none) {
					countRight++;

					this->vecCountRight[(y-72)/parameters.gridStepSize]++;
				}			
				if (pointExplorer.getColor(x,y) == ColorClasses::green || pointExplorer.getColor(x,y) == ColorClasses::orange) {
					countGreenRight++;

					this->vecCountFeasibleRight[(y-72)/parameters.gridStepSize]++;
				}
			}
		}
		y = y+ parameters.gridStepSize;

...
void BallLocator::update(BallModel& ballModel)
{
  ...
  leftFoot.conc(theRobotModel.limbs[MassCalibration::footLeft]).translate(p.footOffset.x, p.footOffset.y, -theRobotDimensions.heightLeg5Joint);
  rightFoot.conc(theRobotModel.limbs[MassCalibration::footRight]).translate(p.footOffset.x, -p.footOffset.y, -theRobotDimensions.heightLeg5Joint);
  ...
  {
    // reset filters..
    if(theFrameInfo.getTimeSince(theBallHypotheses.timeWhenDisappeared) >= 4000)
      reset();

    // calculate variance of the percept
    ballPerceptCov = getCovOfPixelInWorld(theImageCoordinateSystem.toCorrected(theBallPercept.positionInImage), float(theFieldDimensions.ballRadius));
...
void BallLocator::motionUpdate(BallModel& ballModel)
{
  ...
  float deceleration = float(theFieldDimensions.ballFriction) * deltaTime;
...
void BallLocator::handleCollisionWithFeet(const Vector2<>& leftFootCenter, const Vector2<>& rightFootCenter)
{
  ...
  float assumedRadius = p.footRadius + theFieldDimensions.ballRadius;
  ...
  if(leftCollision || rightCollision)
  {
    ...
    float deltaTime = (theFrameInfo.time - lastFrameTime) * 0.001f;
...
void BallLocator::generateModel(BallModel& ballModel)
{
  ...
  if(bestState)
  {
    ...
    if(bestState->type == State::moving)
    {
      ...
      ballModel.endPosition = ballState.position + Vector2<>(ballState.velocity).normalize(ballState.velocity.squareAbs() / (2.f * (float) theFieldDimensions.ballFriction));

...
SensorWidget::SensorWidget(SimRobotCore2::Sensor* sensor) : pen(QColor::fromRgb(255, 0, 0)), sensor(sensor), mineData(0)
{
...
void SensorWidget::paintEvent(QPaintEvent *event)
{
  ...
  switch(sensorType)
  {
  case SimRobotCore2::Sensor::floatSensor:
    {
  ...
  case SimRobotCore2::Sensor::floatArraySensor:
    {
  ...
  case SimRobotCore2::Sensor::boolSensor:
    {
      paintBoolSensor();
      break;
    }
  case SimRobotCore2::Sensor::cameraSensor:
    {
...
void SensorWidget::paintFloatArrayWithDescriptionsSensor()
{
  ...
  SimRobotCore2::Sensor::Data data = sensor->getValue();
  const QStringList& descriptions = sensor->getDescriptions();
  bool conversionToDegreesNeeded = unitForDisplay.indexOf("?") != -1;
  ...
  {   
    ...
    painter.drawText(0, 20*i, this->width(), 20, Qt::AlignRight, 
      QString(valueStream.str().c_str()) + " " + unitForDisplay + " ");
  }
}
...
{
  if(sensorType != SimRobotCore2::Sensor::cameraSensor)
...
void SensorWidget::setClipboardText(QMimeData& mimeData)
{
  ...
  case SimRobotCore2::Sensor::noSensor:
    {
      break;
    }
  }
